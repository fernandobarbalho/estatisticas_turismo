...
dados_grafico_ranking_2025 %>%
  ...
  geom_col(aes(fill= quantidade), show.legend = FALSE) +
  geom_text(aes(label = scales::number(quantidade, decimal.mark = ",", big.mark = ".", accuracy = 1.0)), hjust= -0.1) +
  scale_fill_continuous_sequential(palette = "Heat 2") +
  scale_x_continuous(expand = expansion(mult=c(0.01, 0.1))) +
  theme_vpr(paleta_cores) +
  labs(
    title= "Argentina lidera o ranking de país com maior número de visitantes com 36% do total",
    subtitle = "Quatro dos cinco países com maior número de visitantes são da América do Sul"
  )
  #título: 
  #subtítulo: 
