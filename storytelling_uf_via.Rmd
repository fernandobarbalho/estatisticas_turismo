...
dados_sumarizados_via_uf<-
  chegadas_2014_2025 %>%
  filter(ano == 2025) %>%
  mutate(pais = ifelse(pais %in% c(top_cinco_paises_2025$pais)  , pais, "Demais países"),
...
dados_grafico_via_uf %>%
  ...
  geom_flow(aes(fill = total), show.legend = FALSE) +
  geom_stratum() +
  geom_text(stat = "stratum", aes(label = after_stat(stratum)))  +
  # geom_label(aes(x= pos_x, y=0.1, label="US$ 0,2 bi"), alpha= 0.3, hjust=1) +
  # geom_label(aes(x= pos_x, y=2, label="US$ 3,1 bi"), alpha= 0.3, hjust=1) +
...
pais_sel <-  c("França", "Paraguai") 

dados_grafico_pais_sel<-
  chegadas_2014_2025 %>%
  filter(pais %in% pais_sel) %>%
  summarise(quantidade = sum(chegadas,na.rm = TRUE),
            .by = c(ano, pais) ) 

dados_grafico_pais_sel %>%
  ggplot(aes(x= ano, y= quantidade, fill = pais)) +
  geom_col(position = "dodge") +
  facet_wrap(pais~., scale = "free_y" )

```

```{r fig.dpi= 300}

dados_grafico_series_paises<-
  chegadas_2014_2025 %>%
  mutate(pais = ifelse(pais %in% c(top_cinco_paises_2025$pais) , pais, "Demais países")) %>%
  summarise(quantidade = sum(chegadas, na.rm = TRUE),
            .by = c(ano, pais)) %>%
...
```{r}
dados_grafico_series_fronteira_sn<-
  chegadas_2014_2025 %>%
  mutate(pais = ifelse(pais %in% paises_fronteira_brasil , "Países com fronteira", "Países sem fronteira")) %>%
  summarise(quantidade = sum(chegadas, na.rm = TRUE),
            .by = c(ano, pais)) %>%
  mutate(ano = as.character(ano))
